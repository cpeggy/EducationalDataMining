...
def generate_comparison_chart(df_scores, kinmen_school_code, target_class_name, output_image_path):
    ...
    plt.title(' '.join(plot_title_parts), fontproperties=my_font, fontsize=16)
    plt.xlabel('整體答對率 (0.0 ~ 1.0)', fontproperties=my_font, fontsize=14)
    plt.ylabel('密度', fontproperties=my_font, fontsize=14)
...
def generate_class_heatmap(df_scores, output_file_path):
    ...
    try:
        ...
        sns.heatmap(
            ...
            cbar_kws={'label': '平均整體答對率', 'labelsize': 12} # 色條標籤
        )
        
        plt.title('金門縣八年級各班級平均整體答對率熱力圖', fontsize=16, fontproperties=my_font)
        plt.xlabel('班級代號', fontsize=12, fontproperties=my_font)
        plt.ylabel('學校名稱', fontsize=12, fontproperties=my_font)
        
        # 設定色條標籤字體
        cbar = plt.gcf().axes[-1]  # 取得最後一個axes，即colorbar
        cbar.set_ylabel('平均整體答對率', fontproperties=my_font)
...
def visualize_student_usage_scatter(df):
    ...
    if '平台使用量' in df.columns:
        ...
        # 使用適當的字體設定進行回歸分析繪圖
        sns.regplot(x='平台使用量', y='總得分率', data=df, ci=95)
        
        # 設定圖表標題和標籤，使用my_font
        plt.title('平台使用量與平均得分率關係', fontsize=16, fontproperties=my_font)
        plt.xlabel('平均平台使用量', fontsize=14, fontproperties=my_font)
        plt.ylabel('平均數學得分率', fontsize=14, fontproperties=my_font)
        
        # 為每個資料點添加年級標籤
        for i, row in df.iterrows():
            plt.text(row['平台使用量'], row['總得分率'], f"{row['年級']}年級", 
